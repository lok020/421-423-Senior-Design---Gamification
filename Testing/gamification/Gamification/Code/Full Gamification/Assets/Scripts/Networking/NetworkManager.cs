    ...
    ********************************************************************************/
    ...
    private void Start()
    {
        ...
        try
        {
            ...
            XmlNodeList firstNode = doc.SelectNodes("//Games");
            XmlNode current = firstNode[0].FirstChild;     //current = seeker
            //seeker = loadSeeker(current);
            loadSeeker(current);

            //current = current.NextSibling; //current = conqueror
        ...
        try
        {
            ...
            _webClient = new WebSocket("wss:ccain.eecs.wsu.edu");//(_serverURI);
    ...
    private void loadSeeker(XmlNode game)
    {
        //List<List<string>> data = new List<List<string>>();
        //inventory
        XmlNode inner = game.FirstChild; //inventory
        //List<string> inventory = new List<string>();
        /*foreach (XmlNode item in inner.ChildNodes)
        {
            //inventory.Add(item.Attributes["id"].Value);
            GlobalControl.Instance.entire_inventory
        }//*/
        for(int i = 0; i < inner.ChildNodes.Count; i++)
        {
            GlobalControl.Instance.entire_inventory[i] = int.Parse(inner.ChildNodes[i].Attributes["id"].Value);
        }
        //data.Add(inventory);
        ...
        for (int i = 0; i < inner.ChildNodes.Count; i++)
        {
            GlobalControl.Instance.entire_storage_inventory[i] = int.Parse(inner.ChildNodes[i].Attributes["id"].Value);
        }
        //data.Add(storage);

        //progress
        inner = inner.NextSibling;
        GlobalControl.Instance.quest_progress = int.Parse(inner.Attributes["value"].Value);
        //string progress = inner.Attributes["value"].Value;
        //data.Add(new List<string>() { progress });

        //stats
        inner = inner.NextSibling;
        //List<string> stats = new List<string>();
        //foreach (XmlNode stat in inner.ChildNodes)
        //stats.Add(stat.Attributes["value"].Value);
        GlobalControl.Instance.hp = int.Parse(inner.ChildNodes[0].Attributes["value"].Value);
        GlobalControl.Instance.stam = int.Parse(inner.ChildNodes[2].Attributes["value"].Value);
        GlobalControl.Instance.ins = int.Parse(inner.ChildNodes[3].Attributes["value"].Value);
        GlobalControl.Instance.dex = int.Parse(inner.ChildNodes[1].Attributes["value"].Value);
        //data.Add(stats);

        //rep
        inner = inner.NextSibling;
        GlobalControl.Instance.reputation = int.Parse(inner.Attributes["value"].Value);
        //string rep = inner.Attributes["value"].Value;
        //data.Add(new List<string>() { rep });

        //current quest
        //inner = inner.NextSibling;
        //GlobalControl.Instance.cu
    ...
    private void Update()
    {
        ...
        if (_timer > _updateTick)
        {
            ...
            if (_webClient == null && !RunWithoutServer)
            {
                try
                {
                    _webClient = new WebSocket(_serverURI);//TcpClient(_serverAddress, _tcpPort);
                    GameObject.Find("Error Message").GetComponent<Text>().text = "Connecting to server...";
                    Handshake();
                    GameObject.Find("Error Message").GetComponent<Text>().text = "";
    ...
    private void OnApplicationQuit()
    {
        ...
        XmlNode root = doc.CreateElement("Games");
    ...
    private void saveSeeker(XmlDocument doc, XmlNode root)
    {
        //seeker
        XmlNode seeker_node = doc.CreateElement("Seeker");
        XmlNode inner = doc.CreateElement("Inventory");
        XmlNode nested;// = doc.CreateElement("Item");
        XmlAttribute attribute;// = doc.CreateAttribute("id");
        foreach (int item in GlobalControl.Instance.entire_inventory)//(string item in this.seeker[0])
        {
            nested = doc.CreateElement("Item");
            attribute = doc.CreateAttribute("id");
            attribute.Value = item.ToString();
            nested.Attributes.Append(attribute);
            inner.AppendChild(nested);
        }
        seeker_node.AppendChild(inner);

        //storage
        inner = doc.CreateElement("Storage");
        foreach (int item in GlobalControl.Instance.entire_storage_inventory)//(string item in this.seeker[1])
        {
            nested = doc.CreateElement("Item");
            attribute = doc.CreateAttribute("id");
            attribute.Value = item.ToString();
        ...
        //progress
        inner = doc.CreateElement("Progress");
        attribute = doc.CreateAttribute("value");
        attribute.Value = GlobalControl.Instance.quest_progress.ToString();//seeker[2][0];
        inner.Attributes.Append(attribute);
        seeker_node.AppendChild(inner);

        //stats
        inner = doc.CreateElement("Stats");
        //Hp
        nested = doc.CreateElement("Hp");
        attribute = doc.CreateAttribute("value");
        attribute.Value = GlobalControl.Instance.hp.ToString();//seeker[3][0];
        nested.Attributes.Append(attribute);
        inner.AppendChild(nested);
        //Dex
        nested = doc.CreateElement("Dex");
        attribute = doc.CreateAttribute("value");
        attribute.Value = GlobalControl.Instance.dex.ToString();//seeker[3][1];
        nested.Attributes.Append(attribute);
        inner.AppendChild(nested);
        //Stam
        nested = doc.CreateElement("Stam");
        attribute = doc.CreateAttribute("value");
        attribute.Value = GlobalControl.Instance.stam.ToString();//seeker[3][2];
        nested.Attributes.Append(attribute);
        inner.AppendChild(nested);
        //ins
        nested = doc.CreateElement("Ins");
        attribute = doc.CreateAttribute("value");
        attribute.Value = GlobalControl.Instance.ins.ToString();//seeker[3][3];
        nested.Attributes.Append(attribute);
        inner.AppendChild(nested);
        seeker_node.AppendChild(inner);

        inner = doc.CreateElement("Reputation");
        attribute = doc.CreateAttribute("value");
        attribute.Value = GlobalControl.Instance.reputation.ToString();//seeker[4][0];
        inner.Attributes.Append(attribute);
        seeker_node.AppendChild(inner);

        root.AppendChild(seeker_node);
        //doc.Save();
    }

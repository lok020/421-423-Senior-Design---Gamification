...
public class BoardCreator : MonoBehaviour
{
    ...
    void CreateRoomsAndCorridors()
    {
        ...
        rooms = new Room[dungeon_stats.numRooms.Random];
        ...
        for (int i = 1; i < rooms.Length; i++)
        {
            ...
            if (i < corridors.Length)
            {
                ...
                {
                    corridors[i].ReSetupCorridor(rooms[x], dungeon_stats.corridorLength, dungeon_stats.roomWidth, dungeon_stats.roomHeight, dungeon_stats.columns, dungeon_stats.rows, y, corridors[x].direction);
                    y++;
                    if (y == 4)
                    {
                        y = 1;
    ...
    bool CheckCorridors(int i)
    {
        for (int x = 0; x <= i; x++)
        {
            if (corridors[i].EndPositionX > rooms[x].xPos + 1 && corridors[i].EndPositionX < rooms[x].xPos + rooms[x].roomWidth + 1 && corridors[i].EndPositionY > rooms[x].yPos + 1 && corridors[i].EndPositionY < rooms[x].yPos + rooms[x].roomHeight + 1)
            {
                Debug.Log("Here");

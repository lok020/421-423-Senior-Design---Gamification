    ...
    ********************************************************************************/
    ...
    private void Start()
    {
        ...
        try
        {
            ...
            _webClient = new WebSocket("ws://" + _serverIP + ":" + _tcpPort + "/Client");//(_serverURI);
            Debug.Log("URL: " + _webClient.Url);
            //set up OnMessage
            _webClient.OnMessage += (sender,e) => ReceivedMsg(sender,e);

            //GameObject.Find("Error Message").GetComponent<Text>().text = "Connecting to server...";
            _webClient.Connect();

            //copied from StartConnection()
            _webClient.Send("LOGIN " + _username + " " + _password);
            Debug.Log(_username + " " + _password + " sent");
    ...
    private void ReceivedMsg(object sender, MessageEventArgs e)
    {
        ...
        {
            loginFlag = 2;  //loading start
            Debug.Log("loginFlag = " + loginFlag);
    ...
    private void Update()
    {
        ...
        if (_timer > _updateTick)
        {
            ...
            if (_webClient == null && !RunWithoutServer)
            {
                try
                {
                    _webClient = new WebSocket(_ipString);//TcpClient(_serverAddress, _tcpPort);
                    _webClient.Connect();
                    //GameObject.Find("Error Message").GetComponent<Text>().text = "Connecting to server...";
                    //Handshake();
                    //GameObject.Find("Error Message").GetComponent<Text>().text = "";
    ...
    ********************************************************************************/
    ...
    private int PlayerLogin(string username, string password, string received)//, string bonusCode)
    {
        ...
        */
        ...
        _loginData = JSON.Parse(received);
        ...
        _playersSeeker = _loginData["Seeker"];
